...
char serialRecieved[SERIALBUFFERSIZE];
char sendMessage[SERIALBUFFERSIZE];
...
void loop() {
  ...
  if (radio.receiveDone())
  {
    ...
    {
      sprintf(lcdbuff, "%s", radio.DATA);
      sprintf(RSSIstr, "%ddBm", rssi);
      Serial.println(lcdbuff); //this passes data to MightyHat / RaspberryPi
  ...
  //Begin Serial Handling//
  	if (Serial.available() > 0) 
  	{
   		...
   		if (serialCounter==0) 
   		{
   			if (tempSerial=='^')
   			{
   				serialRecieved[serialCounter]=tempSerial;
   				serialCounter++;
   			}
   		}
   		else
    ...
    {
       	transmitSerial();
       	memset(serialRecieved,0,sizeof(serialRecieved));
       	serialCounter=0;
       	tempSerial='0';
    }
        
    if (serialCounter==(SERIALBUFFERSIZE))
    {
    	memset(serialRecieved,0,sizeof(serialRecieved));
       	serialCounter=0;
       	tempSerial='0';
...
void transmitSerial()
{
	serialCounter=1;  //Move past ^
	
	targetNode=serialRecieved[serialCounter]-'0';
	serialCounter++;
	
	
	if (serialRecieved[serialCounter]!='@')
	{
		targetNode=10*targetNode+(serialRecieved[serialCounter]-'0');
		serialCounter++;
	}
	if (serialRecieved[serialCounter]!='@')
	{
	...
	serialCounter++;  //Array position is @

	messageCounter=0;
	while (serialRecieved[serialCounter]!='*')
	{
		if (serialCounter==SERIALBUFFERSIZE) //Message length too long, aborting.
		{
			serialCounter=0;
			messageCounter=0;
			targetNode=0;
		...
		sendMessage[messageCounter]=serialRecieved[serialCounter];
		serialCounter++;
		messageCounter++;
	}
	
	sprintf(transmitBuffer, "%s", sendMessage);
	sendSize = strlen(transmitBuffer);
	radio.sendWithRetry(targetNode, transmitBuffer, sendSize);
